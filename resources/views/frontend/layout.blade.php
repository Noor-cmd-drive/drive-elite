...
<body class="text-gray-800 antialiased selection:bg-orange-500 selection:text-white flex flex-col min-h-screen transition-all duration-500">
    ...
    @php
        // 🚀 SMART CHECK: Kya is IP ka record admin panel mein zinda hai?
        $hasDbConsent = \App\Models\CookieConsent::where('ip_address', request()->ip())->exists();
    @endphp

    <div id="cookieBanner" class="fixed bottom-0 left-0 right-0 bg-[#070b14] border-t border-orange-500/30 p-4 md:p-6 shadow-[0_-10px_40px_rgba(0,0,0,0.8)] z-50 transform translate-y-full transition-transform duration-500 ease-in-out font-inter" style="display: none;">
        <div class="max-w-7xl mx-auto flex flex-col md:flex-row items-center justify-between gap-6">
            
            <div class="flex-1 text-center md:text-left">
                <h3 class="text-white font-poppins font-black uppercase text-sm md:text-base tracking-widest mb-2 flex items-center justify-center md:justify-start gap-2">
                    <i class="fa-solid fa-cookie-bite text-orange-500"></i> Elite Privacy Preferences
                </h3>
            ...
            <div class="flex flex-wrap justify-center md:justify-end gap-3 w-full md:w-auto">
                <button onclick="handleCookieConsent('reject_all')" class="px-5 py-2.5 rounded-xl border border-white/20 text-gray-300 text-xs font-bold uppercase tracking-wider hover:bg-white/10 transition-colors">
                    Reject All
                </button>
                <button onclick="openCookieModal()" class="px-5 py-2.5 rounded-xl border border-orange-500/50 text-orange-500 text-xs font-bold uppercase tracking-wider hover:bg-orange-500/10 transition-colors">
                    Customize
                </button>
                <button onclick="handleCookieConsent('accept_all')" class="px-6 py-2.5 rounded-xl bg-orange-500 hover:bg-orange-600 text-white text-xs font-black uppercase tracking-wider shadow-[0_5px_15px_rgba(249,115,22,0.4)] transition-all transform hover:scale-105">
                    Accept All
                </button>
            </div>
        </div>
    ...
    <div id="cookieModal" class="fixed inset-0 bg-black/80 backdrop-blur-sm z-[60] flex items-center justify-center opacity-0 pointer-events-none transition-opacity duration-300 font-inter p-4">
        <div class="bg-[#0b1120] border border-white/10 rounded-[2rem] w-full max-w-lg shadow-2xl transform scale-95 transition-transform duration-300" id="cookieModalContent">
            
            <div class="p-6 md:p-8">
                <div class="flex justify-between items-center mb-6">
                    <h3 class="text-white font-poppins font-black uppercase tracking-widest text-lg">Customize Preferences</h3>
                    <button onclick="closeCookieModal()" class="text-gray-500 hover:text-white transition-colors"><i class="fa-solid fa-xmark text-xl"></i></button>
                </div>
                ...
                <div class="space-y-4 max-h-[60vh] overflow-y-auto pr-2 custom-scrollbar">
                    ...
                    <div class="bg-black/30 border border-white/5 p-4 rounded-xl">
                        <div class="flex justify-between items-center mb-2">
                            <h4 class="text-white font-bold text-sm">Strictly Necessary</h4>
                            <span class="text-orange-500 text-xs font-black uppercase tracking-wider">Always Active</span>
                        </div>
                        <p class="text-gray-400 text-xs">These cookies are essential for the website to function properly. They cannot be disabled.</p>
                    </div>

                    <div class="bg-black/30 border border-white/5 p-4 rounded-xl flex justify-between items-start gap-4">
                        <div>
                            <h4 class="text-white font-bold text-sm mb-1">Analytics</h4>
                            <p class="text-gray-400 text-xs">Help us improve by collecting anonymous usage data.</p>
                        </div>
                        <label class="relative inline-flex items-center cursor-pointer mt-1">
                            <input type="checkbox" id="pref_analytics" class="sr-only peer" checked>
                    ...
                    <div class="bg-black/30 border border-white/5 p-4 rounded-xl flex justify-between items-start gap-4">
                        <div>
                            <h4 class="text-white font-bold text-sm mb-1">Marketing</h4>
                            <p class="text-gray-400 text-xs">Used to deliver personalized advertisements.</p>
                        </div>
                        <label class="relative inline-flex items-center cursor-pointer mt-1">
                            <input type="checkbox" id="pref_marketing" class="sr-only peer">
                            <div class="w-9 h-5 bg-gray-600 peer-focus:outline-none rounded-full peer peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:border-gray-300 after:border after:rounded-full after:h-4 after:w-4 after:transition-all peer-checked:bg-orange-500"></div>
                        </label>
                    </div>
                ...
                <div class="mt-8">
                    <button onclick="saveCustomCookies()" class="w-full bg-orange-500 hover:bg-orange-600 text-white font-black uppercase text-sm tracking-widest py-3.5 rounded-xl shadow-[0_5px_15px_rgba(249,115,22,0.4)] transition-all">
                        Save My Choices
                    </button>
                </div>
            </div>
    ...
    <script>
        document.addEventListener("DOMContentLoaded", function() {
            // 🚀 JADOO YAHAN HAI: Laravel se JS mein data pass kiya
            const hasDbConsent = {{ $hasDbConsent ? 'true' : 'false' }};
            
            // Agar Admin ne database se delete kar diya hai, toh browser ki memory bhi ura do!
            if (!hasDbConsent) {
                localStorage.removeItem('elite_cookie_consent');
            }

            // Normal check: Agar memory mein nahi hai, toh banner dikhao
            if (!localStorage.getItem('elite_cookie_consent')) {
                setTimeout(() => {
        ...
        function handleCookieConsent(status, preferences = {}) {
            const banner = document.getElementById('cookieBanner');
            banner.classList.add('translate-y-full');
            setTimeout(() => { banner.style.display = 'none'; }, 500);
            closeCookieModal();

            localStorage.setItem('elite_cookie_consent', status);
            ...
            Toast.fire({ icon: 'success', title: 'Privacy preferences saved.' });
            ...
            .then(data => console.log('Cookie status synced with DB:', data))
        ...
        function saveCustomCookies() {
            ...
            handleCookieConsent('customized', preferences);
        }
    </script>
</body>
</html> 